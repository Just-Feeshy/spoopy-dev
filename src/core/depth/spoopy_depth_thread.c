...
static void sys_thread_try_detach(void* thrd) {
    spoopy_sys_thread_t* raw_thrd = (spoopy_sys_thread_t*)thrd;
...
static void sys_thread_finalize(spoopy_sys_thread_t* thrd) {
...
static void sys_thread_try_finalize(void* thread_buffer) {
    spoopy_sys_thread_t* thrd = (spoopy_sys_thread_t*)thread_buffer;
    if(SDL_CompareAndSwapAtomicInt(&thrd->thread_state,
                                              SPOOPY_THREAD_STATE_FINISHED,
                                              SPOOPY_THREAD_STATE_CLEANUP)) {
        sys_thread_finalize(thrd);
    }
}

static void sys_thread_decref_internal(spoopy_sys_thread_t* thrd) {
    int prev_ref_count = SDL_AddAtomicInt(&thrd->ref_count, -1);
    assert(prev_ref_count > 0);

    if(prev_ref_count == 1) {
        sys_thread_try_finalize(thrd);
    }
}

static int SDLCALL sys_thread_entry(void* data) {
    spoopy_core_thread_data_t core_data = *(spoopy_core_thread_data_t*)data;
    spoopy_thread_id_t id = SDL_GetCurrentThreadID();

    spoopy_sys_thread_t* thrd = (spoopy_sys_thread_t*)core_data.buffers.thread_buffer;
    thrd->id = id;
    spoopy_global_thread_wrapper_t global_thread = {
        .id = id,
        .buffers = core_data.buffers,
        .safely_detach = sys_thread_try_detach,
        .safely_finalize = sys_thread_try_finalize
    };
...
void spoopy_sys_thread_init(void) {
...
void spoopy_sys_thread_shutdown(void) {
    ...
    while(existing_threads) {
        ...
        spoopy_sys_thread_t* thrd = (spoopy_sys_thread_t*)global_thrd->buffers.thread_buffer;
...
spoopy_sys_thread_t* spoopy_sys_thread_create(
    const char* name,
    ...
    spoopy_sys_thread_t* thread = SPOOPY_FLEX_ALLOC(spoopy_sys_thread_t, name_size, spoopy_heap);
...
void* spoopy_sys_thread_wait(spoopy_sys_thread_t* thrd) {
    ...
    void* r = thrd->data;
    sys_thread_decref_internal(thrd);
    return r;
}

bool spoopy_sys_thread_get_result(spoopy_sys_thread_t* thread, void** result) {
...
bool spoopy_sys_current_is_main(void) {
