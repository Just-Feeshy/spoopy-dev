...
typedef struct spoopy_backend_funcs {
	void (*init)(void);
	void (*shutdown)(void);
	bool (*shader_init)(spoopy_shader_object_t* shader, spoopy_shader_source_t* info);
	...
	void (*texture_set)(spoopy_pipeline_t* pipeline, const char* uniform_name, spoopy_texture_t* tex);
