...
static inline bool spoopy_interpolate_##type##(const spoopy_interp_##type##_t* interp, float frac) { \
	if(interp->need_update) { \
		interp->_next = spoopy_lerp_f32(interp->prev, interp->curr, frac); \
		return true; \
	} \
	return false; \
} \
static inline bool spoopy_interp_##type##_equals(const spoopy_interpolate_##type##_t interp, type o) { \
	return interp._curr == o; \
} \
